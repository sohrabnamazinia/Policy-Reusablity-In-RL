...
from heuristic_synthetic import run_heuristic
...
times_heuristic = []
speedups = []
csv_file_name = "Test_Speedup_" + agent_type + "_synthetic_heuristic.csv"

q_tables_path = {}


for i in range(num_synthetic_policies):
    q_tables_path[f"q_table_{i}_output_path"] = f"q_table_R{i}.npy"
...
# setup panda
df = pd.DataFrame()
header = ["Number of Synthetic Rewards", "Time - heuristic", "Time - train scratch", "Speedup_greedy"]
reward_index = 0
...
for i, n in enumerate(synthetic_reward):
    for j in range(n):
        ...
        time, dag, _ = train_q_policy(grid_world=synthetic_env[f"R{j}_env"], n_episodes=n_episodes,
                                                ...
                                                output_path=q_tables_path[f"q_table_{j}_output_path"], result_step_size=result_step_size,
    ...
    max_cumulative_reward, best_path, paths, shortest_paths, time_heuristic = run_heuristic(q_tables_path, heuristic_k, n, heuristic_max_allowed_path_size, gridworld=combined_env)
