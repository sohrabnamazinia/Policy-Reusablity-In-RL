...
# setup panda
df = pd.DataFrame()
header = ["State Space Size", "Avg Reward - ExNonZeroDiscount", "Avg Reward - Train From Scratch"]
env_size_index = 0
...
for i in range(env_test_count):
    ...
    for j in range(diff_start_query_test):
        ...
        best_path, reward_exnonzero, total_time, pruning_percentage = run_pruning(env=combined_env, dag_1=dag_path, dag_2=dag_gold, discount_factor=discount_factor, learning_rate=learning_rate)
